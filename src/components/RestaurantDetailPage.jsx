...
export function RestaurantDetailPage({
  ...
  function getRecommendersForDeal(deal) {
    if (!deal) return []
    const allRecs = recommendationsByDealId[deal.id] ?? []
    const userBookingForDeal = userBookings.find(
      (b) => b.dealId === deal.id,
    )
    const cutoff = userBookingForDeal
      ? (userBookingForDeal.lastActivity ??
        userBookingForDeal.createdAt ??
        0)
      : null
    const eligible =
      cutoff === null
    ...
    return eligible
      .map((r) => {
        ...
        return u
          ? { ...u, message: r.message, createdAt: r.createdAt }
          : null
      })
      .filter(Boolean)
  }
  ...
  return (
    <div className="w-full h-full flex flex-col bg-white relative">
      ...
      {/* Top bar: back button is always visible. When the page has been
          scrolled past the in-flow title + tab block, the centered restaurant
          name fades in and the SubTabs slide down underneath, becoming the
          sticky header. */}
      <div className="shrink-0 relative z-20 bg-white">
        <div className="px-3 py-2 flex items-center relative min-h-13">
          <button
            ...
            className="w-9 h-9 rounded-full bg-surface flex items-center justify-center hover:bg-surface-strong active:scale-95 transition shrink-0 relative z-10"
          >
      ...
      >
        ...
        <div className="mt-4 flex gap-2 overflow-x-auto no-scrollbar pl-4 pr-4">
          <img
            ...
            alt=""
            className="w-72 h-44 rounded-xl object-cover shrink-0"
          />
          <img
            src={restaurant.image}
            alt=""
        ...
        <div className="px-4 mt-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="inline-flex items-center gap-1.5 h-8 px-3 rounded-full bg-surface text-ink-muted text-[12px] font-semibold">
        ...
        <div className="px-4 mt-8">
          ...
          {restaurantReviews.length > 0 ? (
            <>
              ...
              <button
                type="button"
                className="w-full h-12 mt-4 rounded-full bg-surface text-ink font-semibold text-[14px] cursor-default"
              >
                {c.seeAllReviews}
              </button>
            </>
